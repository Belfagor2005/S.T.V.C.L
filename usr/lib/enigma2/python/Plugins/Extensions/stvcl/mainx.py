...
def m3ulistEntry(download):
	...
	if width == 2560:
		...
		res.append(MultiContentEntryPixmapAlphaTest(pos=(5, 0), size=(50, 50), png=loadPNG(png)))
...
class StvclMain(Screen):
	def __init__(self, session):
		...
		self["actions"] = ActionMap(
			["OkCancelActions", "DirectionActions", "HotkeyActions", "InfobarEPGActions", "ChannelSelectBaseActions", "MenuActions"],
			{
				"ok": self.okRun,
				"menu": self.scsetup,
				"back": self.exit,
	...
	def install_update(self, answer=False):
		if answer:
			# Execute update command using shell script from decoded installer_url
	...
	def downlist(self, sel, url):
		...
		try:
			...
			# Sanitize filename
			fileTitle = sub(r'[<>:"/\\|?*\[\]]', '_', namem3u)         # Replace forbidden characters
			fileTitle = sub(r'[\s\(\)#\'\+\!&]', '_', fileTitle)       # Replace spaces and symbols
			fileTitle = sub(r'_+', '_', fileTitle).strip('_').lower()  # Normalize underscores and lowercase
...
class ChannelList(Screen):
			...
			try:
				...
				if isfile(in_tmp) and stat(in_tmp).st_size > 0:
					...
					with open('%s%s' % (dir_enigma2, bqtname), 'w') as outfile:
						...
						for line in open(in_tmp):
							if line.startswith(('http://', 'https')):
								...
								if '<![CDATA[' in line:
									desk_tmp = line.split('[')[-1].split(']')[0] + '\r\n'
								else:
									desk_tmp = line.split('<')[1].split('>')[1] + '\r\n'
					...
					if isfile('/etc/enigma2/bouquets.tv'):
						...
						if not in_bouquets:
							if isfile('%s%s' % (dir_enigma2, bqtname)) and isfile('/etc/enigma2/bouquets.tv'):
		...
		if result:
			try:
				if self.downloading:
					...
					fileTitle = sub(r'_+', '_', fileTitle)  # Rimuove underscore ripetuti
				...
				for name, url in matches:
					if url.startswith("http"):
						...
						if "samsung" in self.url.lower() or cfg.filterx.value:
							sub_regex = r'(.*?)(?:\.m3u8)?$'
							bases = compile(sub_regex, DOTALL).findall(clean_url)
							if bases:
	...
	def runChannel2(self):
		try:
			...
			self.play_that_shit(url, name, self.currentindex, raw_item, self.items)
...
class M3uPlay2(
	...
	def __init__(self, session, name, url, index, item, items):
		...
		if '8088' in str(self.url):
			...
			self.onFirstExecBegin.append(lambda: self.cicleStreamType(self.name, force=True))
	...
	def openPlay(self, servicetype, url, name):
		print("openPlay called with: servicetype={}, url={}, name={}".format(servicetype, url, name))
		name = str(name)
							...
							try:
								try:
									with open(tpicf, 'wb') as f:
										f.write(get(url, stream=True, allow_redirects=True).content)
									print('=============11111111=================\n')
								except Exception as e:
...
class GridMain(Screen):
	def __init__(self, session, names, urls, pics=[]):
		...
		tmpfold = tvstrvl + "/tmp"  # join(str(cfg.cachefold.value), "stvcl/tmp")
		picfold = tvstrvl + " /pic"  # join(str(cfg.cachefold.value), "stvcl/pic")
