...
TMPPATH=/tmp/stvcl-install
FILEPATH=/tmp/stvcl-main.tar.gz

echo "Starting S.T.V.C.L installation..."
...
cleanup() {
    ...
    [ -d "/tmp/stvcl--main" ] && rm -rf "/tmp/stvcl--main"
}
...
if [ -d "$TMPPATH/stvcl--main/usr/lib/enigma2/python/Plugins/Extensions/stvcl" ]; then
    cp -r "$TMPPATH/stvcl--main/usr/lib/enigma2/python/Plugins/Extensions/stvcl"/* "$PLUGINPATH/" 2>/dev/null
    echo "✅ Copied from standard plugin directory"
elif [ -d "$TMPPATH/stvcl--main/usr/lib64/enigma2/python/Plugins/Extensions/stvcl" ]; then
    cp -r "$TMPPATH/stvcl--main/usr/lib64/enigma2/python/Plugins/Extensions/stvcl"/* "$PLUGINPATH/" 2>/dev/null
    echo "✅ Copied from lib64 plugin directory"
elif [ -d "$TMPPATH/stvcl--main/usr" ]; then
    # Copy entire usr tree
    cp -r "$TMPPATH/stvcl--main/usr"/* /usr/ 2>/dev/null
